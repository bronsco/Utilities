...
if tz~=3
	...
	disp('Please wait..................');
	loopCnt = 0;
	hwait = waitbar(0,'0 %%','Name','Image colorization');
	for i=1:mx
		for j=1:my
			tstart = tic;
			iy=dx2(i,j);
			...
			loopCnt = loopCnt + 1;
			tend(loopCnt) = toc(tstart);
			waitbar( loopCnt/(mx*my), hwait, sprintf( '%i of %i done (%3.4f%%, %.0f min remaining)',loopCnt, mx*my, loopCnt/(mx*my)*100, ceil(mean(tend)*(mx*my-loopCnt)/60) ) )
		end
	end
	delete(hwait);
