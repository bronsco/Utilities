...
% get author string
hAuthors = hDoc.getElementsByTagName('authors');
nAuthors = hAuthors.item(0).getLength;
cAuthors = cell(2,nAuthors);
for iAuthor = 1 : nAuthors
  cAuthors{1,iAuthor} = char(hAuthors.item(0).item(iAuthor-1).item(0).item(0).getData);
  if iAuthor == nAuthors
    cAuthors{2,iAuthor} = '';
  else
...
for iDoc = 1 : nDocs
  ...
  if nAuth > 0
    tAuthor = char(hAuth.item(iItem).item(0).item(0).item(0).getData);
  else
    iItem = iItem + 1;
    continue;
  end
