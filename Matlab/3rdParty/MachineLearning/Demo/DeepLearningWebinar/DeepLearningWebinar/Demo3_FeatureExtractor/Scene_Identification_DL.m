...
rootFolder = '../Scene_Categories';
categories = {'Auditorium'  ,  'Beach'  ,  'Field'   , 'Restaurant'};
...
auditorium = find(imds.Labels == categories(1), 1);
beach = find(imds.Labels == categories(2), 1);
field = find(imds.Labels == categories(3), 1);
restaurant = find(imds.Labels == categories(4), 1);
%% Visualize image in each set
figure
subplot(1,4,1);

imshow(imds.Files{auditorium})
title(categories(1));
subplot(1,4,2);

imshow(imds.Files{beach})
title(categories(2));
subplot(1,4,3);
title(categories(3));
imshow(imds.Files{field})
title(categories(3));

subplot(1,4,4);
imshow(imds.Files{restaurant})
title(categories(4));
