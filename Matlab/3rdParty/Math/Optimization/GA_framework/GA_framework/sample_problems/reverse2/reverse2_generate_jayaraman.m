...
while (modifica > 0)
    ...
    for i=1:problem.n_j
        if (ind.p(i) == 0)
            ...
            if(solFinalOtima > fx && melhorF > fx) %se a solução encontrada for melhor que todas as soluções já encontradas
                %fprintf('Melhor solucao encontrada com a HE! Testando se é possível colocá-la\n');
                novo.p;
                if (size(novo.p(novo.p~=0),2) <= (problem.p_max) && size(novo.q(novo.q~=0),2) <= (problem.q_max)) %se for uma solução válida
                    melhor = novo;
                    melhorF = fx;
                    modifica = 1;
                end
    ...
    %se a solução foi modificada
    if (modifica > 0)
        %fprintf('solução melhor encontrada pela expansão heuristica\n');
